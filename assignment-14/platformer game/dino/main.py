    ...
    def on_update(self, delta_time: float):
        ...
        if self.game_over == False:
            ...
            if self.bgcolortime - self.starttime > 15 and self.flag == 0:
                self.starttime = time.time()
                arcade.draw_text(f"{self.score}",650,450,arcade.color.WHITE,25)
            ...
            for cactus in self.enemy_list:
                for bird in self.bird_list:
                    if arcade.check_for_collision(self.player ,cactus) or arcade.check_for_collision(self.player ,bird) :
